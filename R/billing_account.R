...
BillingAccount <- R6::R6Class("BillingAccount", inherit = Document, public = list(address = NULL, 
    firstName = NULL, lastName = NULL, companyName = NULL, taxId = NULL, initialize = function(json = NULL) {
        if (!is.null(json)) {
            self$initJson(json)
        } else {
            self$init()
        ...
        self$taxId = json$taxId
        self$address = createObjectFromJson(json$address)
    }, toTson = function() {
        m = super$toTson()
        m$kind = tson.scalar("BillingAccount")
        ...
        m$taxId = tson.scalar(self$taxId)
        return(m)
    }, print = function(...) {
