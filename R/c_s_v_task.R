...
CSVTask <- R6::R6Class("CSVTask", inherit = Task, public = list(fileDocumentId = NULL, 
    schemaId = NULL, initialize = function(json = NULL) {
        ...
        m = super$toTson()
        m$kind = rtson::tson.scalar(jsonlite::unbox("CSVTask"))
        m$fileDocumentId = rtson::tson.scalar(jsonlite::unbox(self$fileDocumentId))
        m$schemaId = rtson::tson.scalar(jsonlite::unbox(self$schemaId))
        return(m)
    }, print = function(...) {
