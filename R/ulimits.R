...
Ulimits <- R6::R6Class("Ulimits", inherit = Base, public = list(core_file_size = NULL, 
    ...
    max_user_processes = NULL, virtual_memory = NULL, file_locks = NULL, initialize = function(json = NULL) {
        ...
        m = super$toTson()
        m$kind = rtson::tson.scalar("Ulimits")
        m$core_file_size = rtson::tson.int(self$core_file_size)
        m$data_seg_size = rtson::tson.int(self$data_seg_size)
        m$scheduling_priority = rtson::tson.int(self$scheduling_priority)
        m$file_size = rtson::tson.int(self$file_size)
        m$pending_signals = rtson::tson.int(self$pending_signals)
        m$max_locked_memory = rtson::tson.int(self$max_locked_memory)
        m$max_memory_size = rtson::tson.int(self$max_memory_size)
        m$open_files = rtson::tson.int(self$open_files)
        m$pipe_size = rtson::tson.int(self$pipe_size)
        m$message_queues = rtson::tson.int(self$message_queues)
        m$real_time_priority = rtson::tson.int(self$real_time_priority)
        m$stack_size = rtson::tson.int(self$stack_size)
        m$cpu_time = rtson::tson.int(self$cpu_time)
        m$max_user_processes = rtson::tson.int(self$max_user_processes)
        m$virtual_memory = rtson::tson.int(self$virtual_memory)
        m$file_locks = rtson::tson.int(self$file_locks)
        return(m)
    }, print = function(...) {
