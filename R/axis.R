...
Axis <- R6::R6Class("Axis", inherit = Base, public = list(axisExtent = NULL, axisSettings = NULL, 
    graphicalFactor = NULL, initialize = function(json = NULL) {
        ...
        m = super$toTson()
        m$kind = rtson::tson.scalar(jsonlite::unbox("Axis"))
        m$axisExtent = self$axisExtent$toTson()
        m$axisSettings = self$axisSettings$toTson()
        m$graphicalFactor = self$graphicalFactor$toTson()
        return(m)
    }, print = function(...) {
