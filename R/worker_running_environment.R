#' WorkerRunningEnvironment
#'
#' @export
#' @format \code{\link{R6Class}} object, super class \code{\link{RunningEnvironment}}.
#' @field workerMachineId of type String.
WorkerRunningEnvironment <- R6::R6Class("WorkerRunningEnvironment", inherit = RunningEnvironment, 
    public = list(workerMachineId = NULL, initialize = function(json = NULL) {
        if (!is.null(json)) {
            self$initJson(json)
        } else {
            self$init()
        ...
        m = super$toTson()
        m$kind = rtson::tson.scalar("WorkerRunningEnvironment")
