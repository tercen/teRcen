...
RenameRelation <- R6::R6Class("RenameRelation", inherit = Relation, public = list(relation = NULL, 
    inNames = NULL, outNames = NULL, initialize = function(json = NULL) {
        ...
        m = super$toTson()
        m$kind = rtson::tson.scalar(jsonlite::unbox("RenameRelation"))
        m$relation = self$relation$toTson()
        m$inNames = lapply(self$inNames, function(each) rtson::tson.scalar(jsonlite::unbox(each)))
        m$outNames = lapply(self$outNames, function(each) rtson::tson.scalar(jsonlite::unbox(each)))
        return(m)
    }, print = function(...) {
